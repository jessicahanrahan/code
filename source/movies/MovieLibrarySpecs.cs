...
namespace code.movies
{
   ...
   */
    ...
    public abstract class movie_library_concern : Spec.isolate_with<RhinoFakeEngine>.observe<MovieLibrary>
    {
    ...
    public class counting_the_number_of_movies : movie_library_concern
    {
      ...
      Establish c = () =>
        Enumerable.Range(1, 2).for_iteration().each(x => movie_collection.Add(new Movie()));
    ...
    public class sorting_movies : concern_for_searching_and_sorting
    {
      ...
      It by_studio_rating_and_year_published = () =>
      {
        ...
        var other = Compare<Movie>.by(x => x.title, SortDirection.ascending);
